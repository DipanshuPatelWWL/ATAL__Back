...
const addInquiry = async (req, res) => {
  try {
    ...
    if (userType === "vendor") {
      duplicate = await InquiryModel.findOne({
        userType: "vendor",
    ...
    if (duplicate) {
      return res.status(409).json({
        ...
        message:
          userType === "vendor"
            ? "This email or business number is already registered for a vendor inquiry"
            : "This email or registration number is already registered for a company inquiry",
      });
    }
    ...
    const newInquiry = new InquiryModel({
      ...
      registrationNumber:
        userType === "company" ? registrationNumber : undefined,
      message,
      uploadDocument,
    });
    ...
    console.error("Error in addInquiry:", err);
...
const sendResponse = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message: "Response sent successfully and email delivered",
      inquiry,
    });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error sending response", error: error.message });
  }
};
...
const sendResponseAndRegister = async (req, res) => {
  try {
    ...
    if (inquiry.inquiryStatus === "close") {
      return res
        .status(400)
        .json({ success: false, message: "Inquiry already closed" });
    }
