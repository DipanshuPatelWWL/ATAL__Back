...
const createCouponCode = async (req, res) => {
    try {
        ...
        if (!addCouponCode || !addCouponCode.coupon || !addCouponCode.applicableFor) {
        ...
        return res.status(200).json({ success: true, message: "Coupon Code details saves successfully" })
    } catch (error) {
...
const updateCouponCode = async (req, res) => {
    try {
        ...
        const updatedCouponCode = await CouponCode.findByIdAndUpdate(id,
            { coupon, applicableFor },
            { new: true }
        );
        if (!updatedCouponCode) {
            return res.status(400).json({ success: false, message: "Coupon Code not found" })
        } res.status(200).json({ success: true, message: "Coupon Code Update Successfully" })
    } catch (error) {
        console.log(error);
